...
export class AdvertisementContainerComponent {
  ...
  onSubmitBasicInfo(data: BasicStepData){
    this.form.controls.basicStep.setValue(data);
  }

  onSubmitServices(data: Services[]){
    this.form.controls.selectedServices.clear();
    data.forEach((service)=>{
      this.form.controls.selectedServices.push(new FormControl(service) as FormControl<Services>)
    })
  }

  onSubmitRooms(data: Room[]){
    console.log(data)
    this.form.controls.selectedRooms.clear();
    data.forEach((room)=>{
      this.form.controls.selectedRooms.push(new FormControl(room) as FormControl<Room>)
    })
